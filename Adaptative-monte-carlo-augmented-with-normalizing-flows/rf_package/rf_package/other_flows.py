...
class SimpleAffine(nn.Module):
    ...
    def inverse(self, y):
        ...
        inv_log_det_jac = torch.log(det_jac) #why is the shape changed?
        return x, inv_log_det_jac

class LayeredSimpleAffine(nn.Module):
    def __init__(self, data_dim, flow_length = 16):
        super().__init__()

        self.layers = nn.Sequential(
            *(SimpleAffine(data_dim) for _ in range(flow_length)))
    ...
    def inverse(self, y):
        inv_log_det_jac_sum = 0
        for layer in self.layers:
            y, log_det_jac = layer.inverse(y)
